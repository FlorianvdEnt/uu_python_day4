...
from fastloop import rbf_network_cython, rbf_network_cython_nonp, rbf_network_cython_ref
...
t0 = time.time()
rbf_network_cython_nonp(X, beta, theta)
print("Cython no numpy: ", time.time() - t0)


t0 = time.time()
rbf_network_cython_ref(X, beta, theta)
print("Cython ref: ", time.time() - t0)
